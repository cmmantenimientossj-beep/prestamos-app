  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1">Evalúa, aprueba y emite los préstamos generados por los cobradores.</p>
      </div>
          ...
          solicitudes.map(s => (
            <div key={s.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
               <div className="w-full">
                  <h3 className="font-bold text-slate-800 text-lg uppercase">{s.cliente.nombre_apellido}</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3">
                     ...
                     <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                        ...
                        <p className="font-black text-slate-800">{s.cantidad_cuotas} cuotas de ${s.valor_cuota.toLocaleString('es-AR')} <span className="text-slate-400 capitalize">({s.modalidad.toLowerCase()})</span></p>
                     </div>
                  </div>
                  <p className="text-xs text-slate-500 mt-3 font-semibold flex items-center gap-1">Generado por: <span className="bg-slate-100 px-1.5 py-0.5 rounded">{s.cobrador.nombre}</span></p>
               </div>
               <div className="w-full md:w-auto mt-4 md:mt-0 flex justify-end">
                  <ApproveButton 
                    prestamoId={s.id} 
                    clienteVal={s.cliente.nombre_apellido} 
                    cobradorNum={s.cliente.celular || ""} 
